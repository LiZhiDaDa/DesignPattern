 ...
 *  写个例子，打游戏保存血量跟蓝量
 *  其实这些个涉及模式中，每个设计模式都有管理者类，写法更加严谨，但是我就是不想写管理者类。建议大家还是写一下为好
 */
- (void)mementoPattern{
 ...
 *  原型模式用到了java的重要知识拷贝对象，而且是深拷贝，Java中需要实现Cloneable接口，在clone方法中进行操作。
 *  OC中如果一个对象需要被拷贝，他需要实现协议：<NSCopying>、<NSMutableCopying>从名字上我们可以看到，一个协议是用于不可变对象的，
 *  一个协议适用于可变对象的
 */
